...
class PARAPluginManager:
    ...
    def get_all_commands(self) -> List[str]:
        """Obtiene todos los nombres de comandos disponibles desde plugins."""
        try:
            commands = []
            for cmd_name, cmd in self.commands.items():
                if cmd_name not in commands:  # Evitar duplicados de aliases
                    commands.append(cmd_name)
            
            logger.debug(f"Comandos de plugins disponibles: {len(commands)}")
            return commands
            
        except Exception as e:
            logger.error(f"Error obteniendo comandos de plugins: {e}")
            return []
