...
class PARALogManager:
    def __init__(self, db_path: str = None):
        ...
        self.auto_resolution_patterns = {
            ...
            r"Comando desconocido: (.+)": self._resolve_command_typo,
        }
    ...
    def auto_fix_logs(self) -> int:
        """Intenta resolver automáticamente los logs pendientes usando patrones de auto-resolución."""
        fixed = 0
        pending_logs = self.get_pending_logs(limit=100)
        for entry in pending_logs:
            for pattern, resolver in self.auto_resolution_patterns.items():
                if re.search(pattern, entry.message):
                    resolution = resolver(entry)
                    if resolution:
                        self.mark_as_resolved(entry.id, resolution)
                        fixed += 1
                        break
        self._update_metrics()
        return fixed 

    def _resolve_command_typo(self, entry):
        """Usa IA para sugerir el comando correcto si hay un typo en el comando desconocido. Marca como resuelto si la IA sugiere un comando válido."""
        try:
            import ollama
            prompt = f"""
El siguiente mensaje de error proviene de una CLI:
"{entry.message}"
¿Puedes sugerir el comando correcto? Devuelve solo el nombre del comando sugerido, o 'NO SUGERENCIA' si no puedes inferirlo.
"""
            response = ollama.chat(model="llama3", messages=[{"role": "user", "content": prompt}])
            suggestion = response['message']['content'].strip().lower()
            if suggestion != 'no sugerencia' and suggestion != '':
                entry.resolution = f"Sugerido por IA: {suggestion}"
                # Marcar como resuelto en la base de datos
                if hasattr(entry, 'id') and entry.id is not None:
                    self.mark_as_resolved(entry.id, entry.resolution)
                return f"Comando corregido sugerido: {suggestion}"
            else:
                return None
        except Exception as e:
            return None 