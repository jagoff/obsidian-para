    ...
    def get_vault(self, vault_path: str = None, force_interactive: bool = False, silent: bool = False) -> Path | None:
        ...
        if vault_path:
            ...
            if path.is_dir() and (path / '.obsidian').is_dir():
                if not silent:
                    console.print(f"[green]🗄️ Vault especificado: [cyan]{path}[/cyan]")
        ...
        if self.cached_vault and not force_interactive:
            if self.cached_vault.is_dir() and (self.cached_vault / '.obsidian').is_dir():
                if not silent:
                    console.print(f"[green]🗄️ Vault en caché: [cyan]{self.cached_vault}[/cyan]")
        ...
        if auto_vault:
            if not silent:
                console.print(f"[green]🗄️ Vault detectado automáticamente: [cyan]{auto_vault}[/cyan]")
        ...
        if force_interactive:
            ...
            if interactive_vault:
                if not silent:
                    console.print(f"[green]🗄️ Vault seleccionado: [cyan]{interactive_vault}[/cyan]")
